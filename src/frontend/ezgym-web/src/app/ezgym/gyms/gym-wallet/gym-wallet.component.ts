...
import {
  PixTypeEnum,
  WalletModel,
} from 'src/app/ezgym/core/models/wallet.model';
...
export class GymWalletComponent
  extends Store<GymWalletComponentState>
  implements OnInit
{
  ...
  constructor(
    ...
    this.gymManagementStore.store$.subscribe((management) => {
      this.setState((state) => ({
        ...
        ui: {
          ...state.ui,
          keyTypeSelected: state.ui.keyTypeOptions.find(
            (k) => k.value == management.wallet?.pix.type
          ),
        },
  ...
  setupPaymentAccount() {
    if (
      this.state.wallet?.paymentAccount?.paymentAccountStatus ==
      PaymentAccountStatusEnum.Approved
    ) {
      this.modalStore.success({
        title: 'Sua conta ja foi aprovada!',
        description:
          'Caso queira modificar seus dados voce pode clicar no botao continuar',
      ...
      .pipe(
        tap(async (paymentAccountEvent) => {
          if (
            ...
            paymentAccountEvent.paymentAccountStatus ==
              PaymentAccountStatusEnum.Approved
          ) {
            this.preLoader.close();
            this.modalStore.success({
